  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        (window as Window & typeof globalThis).DeviceOrientationEvent &&
        typeof (DeviceOrientationEvent as unknown as Window & typeof globalThis & { requestPermission: () => Promise<void> }).requestPermission === 'function'
      ) {
        try {
          await (DeviceOrientationEvent as unknown as Window & typeof globalThis & { requestPermission: () => Promise<void> }).requestPermission();
        } catch {
    ...
    const start = async () => {
      ...
      const detect = async (ts: number) => {
        ...
        if ('requestVideoFrameCallback' in HTMLVideoElement.prototype) {
          (video as unknown as { requestVideoFrameCallback: (callback: () => void) => void }).requestVideoFrameCallback(() => detect(performance.now()));
        } else {
